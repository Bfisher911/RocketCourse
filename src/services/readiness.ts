import type { CourseProject, ReadinessCheck, ReadinessReport } from "../types";
...
const hasUnsafeHtml = (html: string): boolean =>
  /<script[\s>]/i.test(html) || /\son[a-z]+\s*=/i.test(html) || /<iframe[\s>]/i.test(html) || /<object[\s>]/i.test(html);
...
export const buildReadinessReport = (course: CourseProject): ReadinessReport => {
  ...
  const unsafeBlocks = htmlBlocks(course).filter((block) => hasUnsafeHtml(block.html));
  ...
  const emptyBlocks =
    course.pages.some((page) => page.bodyHtml.trim().length < 160) ||
    course.assignments.some((assignment) => assignment.descriptionHtml.trim().length < 160) ||
    course.discussions.some((discussion) => discussion.promptHtml.trim().length < 160);
  ...
  const checks: ReadinessCheck[] = [
    ...
    check("bloom", "Bloom alignment present", course.outcomes.every((outcome) => Boolean(outcome.bloomLevel)), "Each outcome includes a Bloom taxonomy level.", "recommended"),
    ...
    check("accessibility", "No unsafe Canvas HTML", unsafeBlocks.length === 0, unsafeBlocks.length ? `${unsafeBlocks.length} content block(s) include unsafe HTML.` : "Content avoids scripts, event handlers, iframes, and objects."),
    ...
    check("empty-content", "No thin content blocks", !emptyBlocks, emptyBlocks ? "Some content blocks are too short for an import-ready course shell." : "Pages, assignments, and discussions have substantial body content.")
  ];
