...
// A properly formatted, multi-page PDF rendered from a page's HTML (headings -> subheadings,
// lists -> bullets, paragraphs -> wrapped body), themed to the course. Replaces the old flat
// text-dump that ran every heading and section together on one truncated page.
const buildPagePdf = (course: CourseProject, title: string, bodyHtml: string): string =>
  new PdfDoc()
    .theme(course.theme.accent, course.theme.accentDark)
    .setFooter(`${course.title || "Course"} - ${title}`)
    .title(title)
    .html(bodyHtml)
    .note("Generated by RocketCourse. Review in Canvas before publishing.")
    .build();
